...
def generateRandMod(Mod="Vanilla",
    ...
    randomCycleTitle = ['bp_MUS_TitleScreen.ogg', 'bp_MUS_MilkyWayEXPLORE.ogg', 'bp_MUS_EngiEXPLORE.ogg',
                        'bp_MUS_ColonialBATTLE.ogg', 'bp_MUS_LastStand.ogg', 'bp_MUS_ShrikeEXPLORE.ogg',
                        'bp_MUS_SlugBATTLE.ogg', 'bp_MUS_LostShipBATTLE.ogg']
    if Mod == 'Arsenal+':
        randomCycleTitle += ['bp_MUS_Calm_Explore.ogg', 'bp_MUS_Demiurge.ogg', 'bp_MUS_Incursor.ogg',
                             'bp_MUS_Infinity_Explore.ogg', 'bp_MUS_NewTerranRepublic_Explore.ogg']
    ...
    if randBoss:
        ...
        if Mod == "Arsenal+":
            randomCycleBossArtillery += ['ARTILLERY_BOSS_1_HARD', 'ARTILLERY_BOSS_2_HARD', 'ARTILLERY_BOSS_3_HARD',
                                         ...
                                         'ARTILLERY_BOSS_3_EASY', 'ARTILLERY_BOSS_4_EASY', 'FR_BOSS_MISSILES_HULL',
                                         'FR_BOSS_BOMB', 'FR_BOSS_BEAM_BIO', 'FR_BOSS_BEAM_FIRE', 'FR_BOSS_BEAM_FOCUS',
                                         'FR_BOSS_BEAM_HULL', 'FR_BOSS_BEAM_INDUSTRIAL', 'FR_BOSS_BEAM_HULL',
                                         'FR_BOSS_EFFECTOR', 'FR_BOSS_ION_FLAK', 'FR_BOSS_ION_HEAVY',
                                         'FR_BOSS_ION_PHASE',
                                         'FR_BOSS_ION_STUN', 'FR_BOSS_LASER_ARTILLERY', 'FR_BOSS_LASER_BURST_SCATTER',
                                         'FR_BOSS_LASER_HEAVY', 'FR_BOSS_LASER_HULL', 'FR_BOSS_LASER_LIGHT_ARTILLERY',
                                         'FR_BOSS_MINES', 'FR_BOSS_MISSILES_BREACH', 'FR_BOSS_MISSILES_FIRE',
                                         'FR_BOSS_MISSILES_SWARM', 'FR_BOSS_PLASMA', 'FR_BOSS_SHOTGUN']
    ...
    if balanced is False:
        ...
        randomCycleHealth = randomCycleHealth[0] + randomCycleHealth[1] + randomCycleHealth[2] + randomCycleHealth[3] + \
                            randomCycleHealth[4]

    randomCycleAug = ['DRONE_SPEED', 'SYSTEM_CASING', 'ION_ARMOR', 'CLOAK_FIRE', 'REPAIR_ARM', 'SCRAP_COLLECTOR',
                      'ADV_SCANNERS', 'AUTO_COOLDOWN', 'SHIELD_RECHARGE', 'WEAPON_PREIGNITE', 'FTL_BOOSTER',
                      'FTL_JUMPER', 'DRONE_RECOVERY', 'FTL_JAMMER', 'STASIS_POD', 'O2_MASKS', 'EXPLOSIVE_REPLICATOR',
                      'FIRE_EXTINGUISHERS', 'FLEET_DISTRACTION', 'TELEPORT_HEAL', 'BATTERY_BOOSTER',
                      'DEFENSE_SCRAMBLER', 'BACKUP_DNA', 'HACKING_STUN', 'LIFE_SCANNER', 'ZOLTAN_BYPASS']
    if balanced is False:
        randomCycleAug += ['ROCK_ARMOR', 'CRYSTAL_SHARDS', 'ENERGY_SHIELD', 'NANO_MEDBAY', 'SLUG_GEL', 'CREW_STIMS']
    if Mod == "CE":
        randomCycleAug += ['AE_ADV_THRUSTERS', 'AE_FUEL_RECYCLE', 'AE_AMMO_MANUFACTURER', 'AE_DRONE_MANUFACTURER',
                           'AE_HOLOGRAM_GENERATOR', 'AE_SECTOR_SCANNER', 'AE_SECTOR_MAPPING', 'AE_CLONE_LAB',
                           'AE_PRODUCTION', 'AE_HOLOGRAM_GENERATOR', 'AE_TRADE_BOOSTER', 'AE_SCIENCE', 'AE_BEACON_KIT',
                           'AE_REPAIR_KIT', 'AE_REQUISITION', 'AE_COMBAT_SIMULATOR', 'AE_BOUNTYHUNTER',
                           'AE_WEAPON_JAMMER', 'AE_DRONE_JAMMER', 'AE_TELEPORTER_JAMMER', 'AE_SHIELD_JAMMER',
                           'AE_IRRADIATOR', 'AE_ADV_EFFECTOR', 'AE_NEBULA_SCANNER', 'AE_MIND_SHIELD', 'AE_FIREWALL',
                           'AE_ASB', 'AE_AUG_GENERATOR']
        if balanced is False:
            randomCycleAug += ["TE_GOODS_CIVILIAN_CONSUMER", "TE_GOODS_CIVILIAN_LUXURY", "TE_GOODS_CIVILIAN_FOOD",
                               "TE_GOODS_ENGI_MEDICINE", "TE_GOODS_ENGI_ELECTRONICS", "TE_GOODS_ENGI_MACHINES",
                               "TE_GOODS_ZOLTAN_BEACONS", "TE_GOODS_ZOLTAN_CODEX", "TE_GOODS_ZOLTAN_SHIELDS",
                               "TE_GOODS_ROCK_LITERATURE", "TE_GOODS_ROCK_ORE", "TE_GOODS_ROCK_HULLS",
                               "TE_GOODS_MANTIS_ENGINES", "TE_GOODS_MANTIS_WEAPONS", "TE_GOODS_MANTIS_BOUNTY",
                               "TE_GOODS_SLUG_DRUGS", "TE_GOODS_SLUG_ART", "TE_GOODS_SLUG_ANIMALS",
                               "TE_GOODS_NEBULA_GAS", "TE_GOODS_PIRATE_ARMS", "TE_GOODS_REBEL_CONTRABAND",
                               "TE_GOODS_CRYSTAL_ARTIFACTS", "TE_GOODS_FED_WRECK", "TE_GOODS_HAZARD_ELEMENTS",
                               "TE_GOODS_FED_WRECK", "TE_GOODS_AI_DATA", "TE_GOODS_AI_NANOBOTS",
                               "TE_GOODS_QUARANTINE_DISEASE", "TE_GOODS_INDUSTRIAL_TERRAFORMING",
                               "TE_GOODS_INDUSTRIAL_REACTOR", "TE_GOODS_LANIUS_POLYMERS", "TE_GOODS_GIRDERS",
                               "TE_GOODS_CHEMICALS", "TE_GOODS_TEXTILES", "TE_GOODS_ORGANISMS", "TE_GOODS_O2",
                               "TE_GOODS_H2O", "TE_GOODS_SEEDS", "TE_GOODS_CONDUITS", "TE_GOODS_PRODUCE",
                               "TE_GOODS_ANTIMATTER", "TE_GOODS_GAMES", "TE_GOODS_SENSORS", "TE_GOODS_LIFESUPPORT",
                               "TE_GOODS_ROBOTS", "TE_GOODS_RESEARCH", "TE_GOODS_SOLAR", "TE_GOODS_RELICS",
                               "TE_GOODS_CLONES", "TE_GOODS_AMBRA", "TE_GOODS_GEMS", "TE_GOODS_GLANDS"]
    if Mod == "Arsenal+":
        randomCycleAug += ['TULLY_BOX']
        if balanced is False:
            randomCycleAug += ['INJECTOR_PRC', 'INJECTOR_FRC', 'INJECTOR_BRC', 'INJECTOR_CLD', 'INJECTOR_POW',
                               'INJECTOR_UNI']
    ...
    if balanced is False:
        ...
        randomCycleCost = randomCycleCost[0] + randomCycleCost[1] + randomCycleCost[2] + randomCycleCost[3] + \
                          randomCycleCost[4]
    ...
    if randShipCheck is True:
        ...
        if os.path.exists('source/compatibility/' + str(Mod) + '/playerImgOrder.xml') is True:
            ...
            tree = ET.parse('source/compatibility/Global/playerImgOrder.xml')
        root = tree.getroot()
        for child in root:
            if child.tag == 'playerImgOrder':
                for img in child:
                    if img.tag == 'img':
                        playerImgOrder.append(img.text)
            elif child.tag == 'shipTypeID':
            ...
            elif child.tag == 'shipID':
                if 'mod' in child.attrib:
                    tree2 = ET.parse('source/compatibility/' + child.attrib['mod'] + '/playerImgOrder.xml')
                    root2 = tree2.getroot()
                    for child2 in root2:
                        if child2 == 'shipTypeID':
                            for id2 in child2:
                                if id2.tag == 'id':
                                    playerShipOrder.append(id2.text)
                else:
                    for id in child:
                        if id.tag == 'id':
                            playerShipOrder.append(id.text)

        uniqueShipCount = len(playerImgOrder)

        randomCyclePlayerHull = []
        for x in range(0, uniqueShipCount):
        ...
        for x in range(0, uniqueShipCount):
            if x >= 8:
                ...
                imgCheck = 4
            while (os.path.exists('source/compatibility/Global/img/ship/' + str(playerImgOrder[x]) + str(
                    shipTypeId[imgCheck - 1]) + '_base.png') is True or os.path.exists(
                'source/compatibility/' + str(Mod) + '/img/ship/' + str(playerImgOrder[x]) + str(
                    shipTypeId[imgCheck - 1]) + '_base.png') is True) and imgCheck < len(shipTypeId):
                imgCheck += 1
        ...
        namesOrder = ['ship_PLAYER_SHIP_BIRD', 'ship_PLAYER_SHIP_CIRCLE', 'skip', 'ship_PLAYER_SHIP_ENERGY',
                      'ship_PLAYER_SHIP_MANTIS', 'ship_PLAYER_SHIP_JELLY', 'ship_PLAYER_SHIP_ROCK', 'skip',
                      'ship_PLAYER_SHIP_CRYSTAL', 'skip', ]
        ...
        for x in range(0, len(allNames)):
            if allNames[x] != 'skip':
                ...
                allNames[x] = allNames[0][(4 - allNames.count('skip')) * 3:] + allNames[0][
                                                                               :(4 - allNames.count('skip')) * 3]
        ...
        for x in range(0, len(allLayouts)):
            ...
            if os.path.exists('source/layouts/' + str(Mod)) is True:
                while os.path.exists('source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x]) + '/' + str(
                        playerImgOrder[x]) + str(shipTypeId[layoutCheck]) + '.xml') is True and os.path.exists(
                    'source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x]) + '/' + str(
                        playerImgOrder[x]) + str(
                        shipTypeId[layoutCheck]) + '.txt') is True and crashCheck < 100:
                    layoutCheck += 1
                    crashCheck += 1
            else:
                while os.path.exists(
                        'source/layouts/Vanilla/' + str(playerShipOrder[x]) + '/' + str(playerImgOrder[x]) + str(
                            shipTypeId[layoutCheck]) + '.xml') is True and os.path.exists(
                    'source/layouts/Vanilla/' + str(playerShipOrder[x]) + '/' + str(playerImgOrder[x]) + str(
                        shipTypeId[layoutCheck]) + '.txt') is True and crashCheck < 100:
                    layoutCheck += 1
                    crashCheck += 1
    ...
    if balanced is False:
        ...
        randomCycleWeaponCooldown = randomCycleWeaponCooldown[0] + randomCycleWeaponCooldown[1] + \
                                    randomCycleWeaponCooldown[2] + randomCycleWeaponCooldown[3] + \
                                    randomCycleWeaponCooldown[4] + randomCycleWeaponCooldown[5]
    ...
    if balanced is False:
        ...
        randomCycleWeaponSpeed = randomCycleWeaponSpeed[0] + randomCycleWeaponSpeed[1] + randomCycleWeaponSpeed[2] + \
                                 randomCycleWeaponSpeed[3] + randomCycleWeaponSpeed[4]
    ...
    if balanced is False:
        ...
        randomCycleAugValue = randomCycleAugValue[0] + randomCycleAugValue[1] + randomCycleAugValue[2] + \
                              randomCycleAugValue[3] + randomCycleAugValue[4]
    ...
    if balanced is False:
        ...
        randomCycleDroneSpeed = randomCycleDroneSpeed[0] + randomCycleDroneSpeed[1] + randomCycleDroneSpeed[2] + \
                                randomCycleDroneSpeed[3] + randomCycleDroneSpeed[4]
    ...
    if balanced is False:
        ...
        randomCycleDroneCooldown = randomCycleDroneCooldown[0] + randomCycleDroneCooldown[1] + randomCycleDroneCooldown[
            2] + randomCycleDroneCooldown[3] + randomCycleDroneCooldown[4] + randomCycleDroneCooldown[5]
    ...
    if extraEquipmentCheck is True:
        ...
        for pack in root:
            try:
                if os.path.exists('source/Extra Packs/' + pack.attrib['name']) is True and (
                        (hyperspace is False and 'hyperspaceReq' not in [
                            elem.tag for elem in
                            pack.iter() if
                            elem is not pack]) or hyperspace is True):
                    chance = 0
                    for child in pack:
                        if child.tag == 'chance':
                            chance = child.text
    ...
    if randShipCheck:
                ...
                if blueprintList.attrib['name'] == 'all_weapons':
                    for weapon in blueprintList:
                        if len(randomCycleWeaponPower) > 2:
                            ...
                            itemPowers[weapon.text] = randomCycleWeaponPower[
                                randomCycleWeaponPower.index(weapon.attrib['power']) - 1]
        ...
        for x in range(0, len(playerShipOrder)):
            ...
            crewList = []
            if balanced is True:
                crewList += STCrewRarities[x % uniqueShipCount]

            crewCount = int(choice(CrewCountRarities[x % uniqueShipCount]))

            while len(crewList) < crewCount:
                shuffle(CrewRarities[x % uniqueShipCount])
                if CrewRarities[x % uniqueShipCount][0] in dlcItems and (x == 9 or x >= 19 or balanced is False):
                    crewList.append(CrewRarities[x % uniqueShipCount][0][:-4])
                if CrewRarities[x % uniqueShipCount][0] not in dlcItems:
                    crewList.append(CrewRarities[x % uniqueShipCount][0])

            if os.path.exists(
                    'source/layouts/' + str(Mod) + '/' + str(playerShipOrder[trueX % uniqueShipCount]) + '/' + str(
                        allLayouts[trueX % uniqueShipCount][trueX // uniqueShipCount]) + '_blue.xml') is True:
                xmlRead = open(
                    'source/layouts/' + str(Mod) + '/' + str(playerShipOrder[trueX % uniqueShipCount]) + '/' + str(
                        allLayouts[trueX % uniqueShipCount][trueX // uniqueShipCount]) + '_blue.xml', 'r')
            else:
                xmlRead = open('source/layouts/Vanilla/' + str(playerShipOrder[trueX % uniqueShipCount]) + '/' + str(
                    allLayouts[trueX % uniqueShipCount][trueX // uniqueShipCount]) + '_blue.xml', 'r')
            ...
            probCount = 0
            systemCount = 8
            for y in range(0, len(systemCountRarities[x % uniqueShipCount])):
                probCount += int(systemCountRarities[x % uniqueShipCount][y])
                if randint(0, sum(systemCountRarities[x % uniqueShipCount])) < probCount:
                    systemCount = y
                    break
            ...
            while len(systems) != systemCount and crashCount < 100:
                if len(systems) < systemCount:
                    for y in range(0, len(systemNames[x % uniqueShipCount])):
                        if systemRarities[x % uniqueShipCount][y] >= randint(0, 100) > 0 and \
                                systemNames[x % uniqueShipCount][
                                    y] not in systems and (
                                systemNames[x % uniqueShipCount][
                                    y] not in dlcItems or x == 9 or x >= 19 or balanced is False):
                            systems.append(systemNames[x % uniqueShipCount][y])
                if len(systems) > systemCount:
                    for y in range(0, len(systems)):
                        if systemRarities[x % uniqueShipCount][
                            systemNames[x % uniqueShipCount].index(systems[y])] < randint(0,
                                                                                          100):
                            systems[y] = 'delete'
            ...
            for y in range(0, len(systems)):
                systemLevels.append(
                    choice(systemLevelWeights[x % uniqueShipCount][systemNames[x % uniqueShipCount].index(systems[y])]))
            ...
            while weaponsPowerSum < weaponSystemLevel and crashCheckWeapon < 10000:
                ...
                if balanced is True and len(weapons) == 0:
                    shuffle(STshipWeapons[x % uniqueShipCount])
                    for y in range(0, len(STshipWeapons[x % uniqueShipCount])):
                        if int(itemPowers[
                                   STshipWeapons[x % uniqueShipCount][y]]) <= weaponSystemLevel - weaponsPowerSum and (
                                STshipWeapons[x % uniqueShipCount][y] not in weapons or balanced is False) and (
                                STshipWeapons[x % uniqueShipCount][
                                    y] not in dlcItems or x == 9 or x >= 19 or balanced is False):
                            weapons.append(STshipWeapons[x % uniqueShipCount][y])
                            weaponsPowerSum += int(itemPowers[STshipWeapons[x % uniqueShipCount][y]])
                            break
                else:
                    shuffle(shipWeapons[x % uniqueShipCount])
                    for y in range(0, len(shipWeapons[x % uniqueShipCount])):
                        if int(itemPowers[
                                   shipWeapons[x % uniqueShipCount][y]]) <= weaponSystemLevel - weaponsPowerSum and (
                                shipWeapons[x % uniqueShipCount][y] not in weapons or balanced is False) and (
                                shipWeapons[x % uniqueShipCount][
                                    y] not in dlcItems or x == 9 or x >= 19 or balanced is False):
                            weapons.append(shipWeapons[x % uniqueShipCount][y])
                            weaponsPowerSum += int(itemPowers[shipWeapons[x % uniqueShipCount][y]])
                            break
            ...
            if 'drones' in systems:
                ...
                while dronesPowerSum < droneSystemLevel + randint(-1, 1) and crashCheckDrone < 1000:
                    if balanced is True and len(drones) == 0:
                        shuffle(STshipDrones[x % uniqueShipCount])
                        for y in range(0, len(STshipDrones[x % uniqueShipCount])):
                            if int(itemPowers[
                                       STshipDrones[x % uniqueShipCount][y]]) <= droneSystemLevel - dronesPowerSum and (
                                    STshipDrones[x % uniqueShipCount][y] not in drones or balanced is False) and (
                                    STshipDrones[x % uniqueShipCount][
                                        y] not in dlcItems or x == 9 or x >= 19 or balanced is False):
                                drones.append(STshipDrones[x % uniqueShipCount][y])
                                dronesPowerSum += int(itemPowers[STshipDrones[x % uniqueShipCount][y]])
                                break
                    else:
                        shuffle(shipDrones[x % uniqueShipCount])
                        for y in range(0, len(shipDrones[x % uniqueShipCount])):
                            if int(itemPowers[
                                       shipDrones[x % uniqueShipCount][y]]) <= droneSystemLevel - dronesPowerSum and (
                                    shipDrones[x % uniqueShipCount][y] not in drones or balanced is False) and (
                                    shipDrones[x % uniqueShipCount][
                                        y] not in dlcItems or x == 9 or x >= 19 or balanced is False):
                                drones.append(shipDrones[x % uniqueShipCount][y])
                                dronesPowerSum += int(itemPowers[shipDrones[x % uniqueShipCount][y]])
                                break

                    crashCheckDrone += 1

            artilleryWeapon = ''
            if trueX % uniqueShipCount == 2:
                artilleryWeapon = choice(ArtilleryChoices[trueX % uniqueShipCount])
            else:
                artilleryWeapon = choice(ArtilleryChoices[x % uniqueShipCount])

            augmentList = []

            shuffle(STshipAug[x % uniqueShipCount])
            for y in range(0, len(STshipAug[x % uniqueShipCount])):
                if STshipAug[x % uniqueShipCount][y] not in dlcItems or x == 9 or x >= 19 or balanced is False:
                    augmentList.append(STshipAug[x % uniqueShipCount][y])

            probCount = 0
            randNumb = randint(0, sum(augCountRarities[x % uniqueShipCount]))
            for y in range(0, len(augCountRarities[x % uniqueShipCount])):
                probCount += int(augCountRarities[x % uniqueShipCount][y])
            ...
            while len(augmentList) < augmentCount and crashCheck < 100:
                shuffle(shipAug[x % uniqueShipCount])
                for y in range(0, len(shipAug[x % uniqueShipCount])):

                    if len(augmentList) >= augmentCount:
                        break

                    if shipAug[x % uniqueShipCount][y] not in augmentList and (
                            shipAug[x % uniqueShipCount][y] not in dlcItems or x == 9 or x >= 19 or balanced is False):
                        augmentList.append(shipAug[x % uniqueShipCount][y])
            ...
            for y in range(0, len(systems)):
                if systems[y] != 'pilot' and systems[y] != 'battery' and systems[y] != 'doors' and systems[
                    y] != 'sensors' and systems[y] != 'medbay':
...
<mod:findLike limit="1" reverse="true"> 
	...
	<mod:setAttributes layout="''' + str(allLayouts[trueX % uniqueShipCount][trueX // uniqueShipCount]) + '''"/>
	<mod:setAttributes img="''' + str(playerImgOrder[trueX % uniqueShipCount]) + str(
                shipTypeId[int(randomCyclePlayerHull[trueX % uniqueShipCount][trueX // uniqueShipCount]) - 1]) + '''"/>
	<mod-overwrite:name>''' + str(allNames[trueX % uniqueShipCount][trueX // uniqueShipCount]) + '''</mod-overwrite:name>
	...
	<mod-overwrite:floorImage>''' + allLayouts[trueX % uniqueShipCount][trueX // uniqueShipCount] + '''</mod-overwrite:floorImage>
	<mod-overwrite:cloakImage>''' + str(playerImgOrder[trueX % uniqueShipCount]) + '''</mod-overwrite:cloakImage>
	<mod-overwrite:shieldImage>''' + str(playerImgOrder[trueX % uniqueShipCount]) + '''</mod-overwrite:shieldImage>
...
'''
            ...
            blueprints_ships += '''
	<mod-append:weaponList count="''' + str(len(weapons)) + '''" missiles="''' + str(
                missileWeaponsCheck * randint(12, 16)) + '''">
...
'''
            ...
            if balanced is True:
                blueprints_ships += '''
	<mod-append:droneList count="''' + str(len(drones)) + '''" drones="''' + str(
                    randint(len(drones) * 8, (len(drones) + 1) * 8) + systems.count('hacking') * randint(6, 12)) + '''">
...
<mod:findWithChildLike type="weaponBlueprint" child-type="cooldown">
	...
	<mod:findWithChildLike type="boost" child-type="count">
		...
		<mod-overwrite:amount>''' + str(float(randomCycleWeaponCooldown[x]) / y - uniform(0, float(
                    randomCycleWeaponCooldown[x]) / y) * 10 // 1 / 10) + '''</mod-overwrite:amount>
...
<!-- minimum sector appearances -->'''
    ...
    sector_data_2 = '<!-- MaxCycle:' + str(randomCycleSectorEventMax) + ' RarityCycle:' + str(
        randomCycleBlueprintRarity) + '''-->
<mod:findComposite>
	<mod:par op="OR">
...
'''
	...
	<discord enabled="true">
		<miniships>\n'''
            ...
            for x in range(0, 28):
                hyperspace_discord += '\t\t<miniship>miniship_' + str(playerImgOrder[x % uniqueShipCount]) + str(
                    shipTypeId[
                        int(randomCyclePlayerHull[x % uniqueShipCount][x // uniqueShipCount]) - 1]) + '</miniship>\n'
    ...
    if extraEquipmentCheck:
        ...
        for x in range(0, len(extraEquipment)):
            ...
            if extraEquipment[x] in [elem.attrib['name'] for elem in root if
                                     'patchAfter' not in [elem2.tag for elem2 in elem if elem2 is not elem]]:
                ...
                for z in range(0, len(extraEquipmentFileTransfer[x])):
                    extraEquipmentFileTransfer[x][z][0] = extraEquipmentFileTransfer[x][z][0].replace('\\',
                                                                                                      '/').replace('//',
                                                                                                                   '/')
        ...
        for x in range(0, len(extraEquipmentFileTransfer[z])):
            for y in range(0, len(extraEquipmentFileTransfer[z][x][2])):
                if extraEquipmentFileTransfer[z][x][2][y][-11:] == '.xml.append':
                    fileAppenda = open(
                        extraEquipmentFileTransfer[z][x][0] + '/' + extraEquipmentFileTransfer[z][x][2][y], 'r')
                    fileAppendb = open('compiledFiles' + extraEquipmentFileTransfer[z][x][0][
                                                         len(extraEquipmentFileTransfer[z][0][0]):] + '/' +
                                       extraEquipmentFileTransfer[z][x][2][y], 'a')

                    fileAppendb.write(fileAppenda.read(10000000))

                    fileAppenda.close()
                    fileAppendb.close()
                elif extraEquipmentFileTransfer[z][x][2][y] != 'AppendArmaments.xml' and \
                        extraEquipmentFileTransfer[z][x][2][y].split('.')[-1] not in ['pdn', 'xcf']:
                    if extraEquipmentFileTransfer[z][x][0][len(extraEquipmentFileTransfer[z][0][0]):][:1] == '/':
                        if not os.path.exists('compiledFiles' + extraEquipmentFileTransfer[z][x][0][
                                                                len(extraEquipmentFileTransfer[z][0][0]):]):
                            os.makedirs('compiledFiles' + extraEquipmentFileTransfer[z][x][0][
                                                          len(extraEquipmentFileTransfer[z][0][0]):])
                        shutil.copy(extraEquipmentFileTransfer[z][x][0] + '/' + extraEquipmentFileTransfer[z][x][2][y],
                                    'compiledFiles' + extraEquipmentFileTransfer[z][x][0][
                                                      len(extraEquipmentFileTransfer[z][0][0]):])
                    else:
                        if not os.path.exists('compiledFiles/' + extraEquipmentFileTransfer[z][x][0][
                                                                 len(extraEquipmentFileTransfer[z][0][0]) - 1:]):
                            os.makedirs('compiledFiles/' + extraEquipmentFileTransfer[z][x][0][
                                                           len(extraEquipmentFileTransfer[z][0][0]) - 1:])
                        shutil.copy(extraEquipmentFileTransfer[z][x][0] + '/' + extraEquipmentFileTransfer[z][x][2][y],
                                    'compiledFiles/' + extraEquipmentFileTransfer[z][x][0][
        ...
        if randShipCheck is True:
            for x in range(0, uniqueShipCount):
                for y in range(0, 2 + int(x < 8)):
                    if os.path.exists('source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x]) + '/' + str(
                            allLayouts[x][y]) + '.txt') is True:
                        shutil.copy('source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x]) + '/' + str(
                            allLayouts[x][y]) + '.txt', 'compiledFiles/data')
                    else:
                        shutil.copy(
                            'source/layouts/Vanilla/' + str(playerShipOrder[x]) + '/' + str(allLayouts[x][y]) + '.txt',
                            'compiledFiles/data')

            for x in range(0, uniqueShipCount):
                for y in range(0, 2 + int(x < 8)):
                    if os.path.exists('source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x]) + '/' + str(
                            allLayouts[x][y]) + '.xml') is True:
                        shutil.copy('source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x]) + '/' + str(
                            allLayouts[x][y]) + '.xml', 'compiledFiles/data')
                    else:
                        shutil.copy(
                            'source/layouts/Vanilla/' + str(playerShipOrder[x]) + '/' + str(allLayouts[x][y]) + '.xml',
                            'compiledFiles/data')
            ...
            for x in range(0, uniqueShipCount):
                if os.path.exists('source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x])) is True:
                    for y in range(0, 2 + int(x < 8)):
                        shutil.copy('source/layouts/' + str(Mod) + '/' + str(playerShipOrder[x]) + '/' + str(
                            allLayouts[x][y]) + '_floor.png', 'compiledFiles/img/ship')
                else:
                    for y in range(0, 2 + int(x < 8)):
                        shutil.copy('source/layouts/Vanilla/' + str(playerShipOrder[x]) + '/' + str(
                            allLayouts[x][y]) + '_floor.png', 'compiledFiles/img/ship')

        for x in range(0, len(transferFiles)):
            for y in range(0, len(transferFiles[x][2])):
                if transferFiles[x][2][y].split('.')[-1] not in ['pdn', 'xcf'] and transferFiles[x][2][y] not in [
                    'playerImgOrder.xml']:
                    if transferFiles[x][2][y][-11:] == '.xml.append':
                        if 'boss' in transferFiles[x][2][y] and randBoss:
                            fileAppenda = open(transferFiles[x][0] + '/' + transferFiles[x][2][y], 'r')
                            fileAppendb = open(
                                'compiledFiles/data/' + transferFiles[x][0][len(transferFiles[0][0]):] + '/' +
                                transferFiles[x][2][y], 'a')
                            ...
                            fileAppenda = open(transferFiles[x][0] + '/' + transferFiles[x][2][y], 'r')
                            fileAppendb = open('compiledFiles/' + transferFiles[x][0][len(transferFiles[0][0]):] + '/' +
                                               transferFiles[x][2][y], 'a')
                            ...
                            fileAppenda.close()
                            fileAppendb.close()
                    elif 'boss' in transferFiles[x][2][y] and transferFiles[x][2][y][-4:] == '.txt' and randBoss:
                        shutil.copy(transferFiles[x][0] + '/' + transferFiles[x][2][y],
                                    'compiledFiles/data' + transferFiles[x][0][len(transferFiles[0][0]):] + '/' +
                                    transferFiles[x][2][y])
                    elif ('_shields1' in transferFiles[x][2][y] or '_base' in transferFiles[x][2][y] or '_gib' in
                          transferFiles[x][2][y] or 'miniship' in transferFiles[x][2][y]) and 'boss' not in \
                            transferFiles[x][2][y]:
                        if randShipCheck is True:
                            for z in range(0, len(randomCyclePlayerHull)):
                                if str(playerImgOrder[z]) in transferFiles[x][2][y]:
                                    for a in range(1, len(randomCyclePlayerHull[z]) + 1):
                                        if str(a) in randomCyclePlayerHull[z][:3 - int(z > 7)] and str(
                                                shipTypeId[a - 1]) in transferFiles[x][2][y][:-5] and '_gib' in \
                                                transferFiles[x][2][y]:
                                            if transferFiles[x][0][len(transferFiles[0][0]):][:1] == '/':
                                                if os.path.exists('compiledFiles' + transferFiles[x][0][len(
                                                        transferFiles[0][0]):]) == False:
                                                    os.makedirs('compiledFiles' + transferFiles[x][0][
                                                                                  len(transferFiles[0][0]):])
                                                shutil.copy(transferFiles[x][0] + '/' + transferFiles[x][2][y],
                                                            'compiledFiles' + transferFiles[x][0][
                                                                              len(transferFiles[0][0]):])
                                            else:
                                                if os.path.exists('compiledFiles/' + transferFiles[x][0][len(
                                                        transferFiles[0][0]):]) == False:
                                                ...
                                                shutil.copy(transferFiles[x][0] + '/' + transferFiles[x][2][y],
                                                            'compiledFiles/' + transferFiles[x][0][
                                                                               len(transferFiles[0][0]):])
                                        elif str(a) in randomCyclePlayerHull[z][:3 - int(z > 7)] and str(
                                                shipTypeId[a - 1]) in transferFiles[x][2][y] and '_gib' not in \
                                                transferFiles[x][2][y]:
                                            if transferFiles[x][0][len(transferFiles[0][0]):][:1] == '/':
                                                if os.path.exists('compiledFiles' + transferFiles[x][0][len(
                                                        transferFiles[0][0]):]) == False:
                                                ...
                                                if os.path.exists('compiledFiles/' + transferFiles[x][0][len(
                                                        transferFiles[0][0]) - 1:]) == False:
                                                    os.makedirs('compiledFiles/' + transferFiles[x][0][
                                                                                   len(transferFiles[0][0]) - 1:])
                                                shutil.copy(transferFiles[x][0] + '/' + transferFiles[x][2][y],
                                                            'compiledFiles/' + transferFiles[x][0][
                                                                               len(transferFiles[0][0]) - 1:])
                    else:
                        if transferFiles[x][0][len(transferFiles[0][0]):][:1] == '/':
                            if os.path.exists(
                                    'compiledFiles' + transferFiles[x][0][len(transferFiles[0][0]):]) == False:
                                os.makedirs('compiledFiles' + transferFiles[x][0][len(transferFiles[0][0]):])
                            shutil.copy(transferFiles[x][0] + '/' + transferFiles[x][2][y],
                                        'compiledFiles' + transferFiles[x][0][len(transferFiles[0][0]):])
                        else:
                            if os.path.exists(
                                    'compiledFiles/' + transferFiles[x][0][len(transferFiles[0][0]) - 1:]) == False:
                                os.makedirs('compiledFiles/' + transferFiles[x][0][len(transferFiles[0][0]) - 1:])
                            shutil.copy(transferFiles[x][0] + '/' + transferFiles[x][2][y],
                                        'compiledFiles/' + transferFiles[x][0][len(transferFiles[0][0]) - 1:])
    ...
    if extraEquipmentCheck:
        ...
        for x in range(0, len(extraEquipment)):
            ...
            if extraEquipment[x] in [elem.attrib['name'] for elem in root if
                                     'patchAfter' in [elem2.tag for elem2 in elem if elem2 is not elem]]:
                for file in os.walk("source/Extra Packs/" + extraEquipment[x]):
                    extraEquipmentFileTransfer[x].append(list(file))
                ...
                for y in range(0, len(extraEquipmentFileTransfer[z][x][2])):
                    if extraEquipmentFileTransfer[z][x][2][y][-11:] == '.xml.append':
                        ...
                        fileAppendb.close()
                            ...
                            extraEquipmentFileTransfer[z][x][2][y].split('.') not in ['pdn', 'xcf']:
                        if extraEquipmentFileTransfer[z][x][0][len(extraEquipmentFileTransfer[z][0][0]):][:1] == '/':
                            if os.path.exists('compiledFiles' + extraEquipmentFileTransfer[z][x][0][
                                                                len(extraEquipmentFileTransfer[z][0][0]):]) == False:
                                os.makedirs('compiledFiles' + extraEquipmentFileTransfer[z][x][0][
                                                              len(extraEquipmentFileTransfer[z][0][0]):])
                            shutil.copy(
                                extraEquipmentFileTransfer[z][x][0] + '/' + extraEquipmentFileTransfer[z][x][2][y],
                                'compiledFiles' + extraEquipmentFileTransfer[z][x][0][
                                                  len(extraEquipmentFileTransfer[z][0][0]):])
                        else:
                            if os.path.exists('compiledFiles/' + extraEquipmentFileTransfer[z][x][0][len(
                                    extraEquipmentFileTransfer[z][0][0]) - 1:]) == False:
                                os.makedirs('compiledFiles/' + extraEquipmentFileTransfer[z][x][0][
                                                               len(extraEquipmentFileTransfer[z][0][0]) - 1:])
                            shutil.copy(
                                extraEquipmentFileTransfer[z][x][0] + '/' + extraEquipmentFileTransfer[z][x][2][y],
